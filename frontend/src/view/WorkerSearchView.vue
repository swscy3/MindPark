...
const openModal = async (worker) => {
 ...
 try {
   ...
   if (detailData.status === 'success' && detailData.data) {
     const apiData = detailData.data;
     
     console.log('=== 상세 데이터 ===');
     console.log('detailData 전체:', detailData);
     console.log('detailData.data:', detailData.data);
     console.log('apiData 전체:', apiData);
     console.log('basic_info 존재 여부:', !!apiData.basic_info);
     console.log('basic_info 객체:', apiData.basic_info);
     console.log('이름 직접 접근:', detailData.data.basic_info.name);
     console.log('이름:', apiData.basic_info?.name);
     console.log('나이:', apiData.basic_info?.age);
     console.log('성별:', apiData.basic_info?.gender);
     console.log('긴급연락처:', apiData.contact_info?.emergency_contact);
     
     selectedWorker.value = {
       // 기본 정보
       id: worker.id,
       name: apiData.basic_info?.name || worker.name,
       code: worker.code,
       department: worker.department,
       position: worker.position,
       status: apiData.attendance_info?.status === "출근중" ? "출근" : "미출근",
       
       // 상세 정보
       gender: apiData.basic_info?.gender === 'M' ? '남성' : apiData.basic_info?.gender === 'F' ? '여성' : '미제공',
       age: apiData.basic_info?.age ? apiData.basic_info.age.toString() : '미제공',
       
       // 생체 정보
       heartRate: apiData.health_data?.heart_rate ? apiData.health_data.heart_rate.toString() : '미측정',
       oxygenSaturation: apiData.health_data?.spo2 ? apiData.health_data.spo2.toString() : '미측정',
       temperature: apiData.health_data?.temperature ? apiData.health_data.temperature.toString() : '미측정',
       steps: apiData.health_data?.steps ? apiData.health_data.steps.toString() : '미측정',
       
       // 디바이스 정보
       deviceLocation: apiData.device_info?.device_name || '미연결',
       batteryLevel: apiData.device_info?.battery_level ? apiData.device_info.battery_level.toString() : '미측정',
       
       // 긴급연락처 정보
       emergencyContact: apiData.contact_info?.emergency_contact ? (() => {
         const contact = apiData.contact_info.emergency_contact;
       ...
       // 위치 정보
       location: {
         latitude: apiData.location_info?.latitude,
         longitude: apiData.location_info?.longitude,
         building: apiData.location_info?.latitude && apiData.location_info?.longitude ? '위치 확인됨' : '위치 미확인',
         floor: '미제공',
         room: '미제공'
       }
     };
...
<style src="../css/WorkerSearch.css"></style>