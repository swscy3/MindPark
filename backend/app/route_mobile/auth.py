...
mobile_auth_bp = Blueprint('auth', __name__)

@mobile_auth_bp.route('/app/login', methods=['POST'])
def app_login():  # 함수 이름을 더 명확하게 app_login으로 변경
    """
    앱 로그인 API
...
@auth_bp.route('/app/logout', methods=['POST'])
@jwt_required()
def app_logout():
    """
    앱 로그아웃 API
