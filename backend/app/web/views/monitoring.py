...
# 🔧 추가: 디버깅용 엔드포인트
@monitoring_bp.route('/debug/device-status')
def debug_device_status():
    ...
    try:
        ...
        # 모든 디바이스 관리 레코드 조회
        all_managements = DeviceManagement.query.all()
        
        debug_records = []
        active_devices = set()
        
        for dm in all_managements:
            if dm.check_in:
                check_in_korea_date = utc_to_korea_date(dm.check_in)
                check_out_korea_date = utc_to_korea_date(dm.check_out) if dm.check_out else None
                
                is_active = (check_in_korea_date == korea_today and dm.check_out is None)
                if is_active:
                    active_devices.add(dm.device_id)
                
                debug_records.append({
                    "device_id": dm.device_id,
                    "check_in_utc": dm.check_in.isoformat() if dm.check_in else None,
                    "check_in_korea": to_korea_time(dm.check_in),
                    "check_in_korea_date": check_in_korea_date.isoformat() if check_in_korea_date else None,
                    "check_out_utc": dm.check_out.isoformat() if dm.check_out else None,
                    "check_out_korea": to_korea_time(dm.check_out),
                    "check_out_korea_date": check_out_korea_date.isoformat() if check_out_korea_date else None,
                    "is_today_korea": check_in_korea_date == korea_today if check_in_korea_date else False,
                    "is_active": is_active
                })
        ...
        debug_info = {
            ...
            "device_summary": {
                ...
                "inactive_devices": total_devices - len(active_devices)
            },
            "device_records": debug_records,
            "active_device_ids": list(active_devices)
        }
        
        return jsonify(debug_info), 200
        
    except Exception as e:
        print(f"[ERROR] 디바이스 상태 디버깅 오류: {str(e)}")
        return jsonify({
            "error": "디바이스 상태 디버깅 중 오류가 발생했습니다.",
            "details": str(e)
        }), 500