...
# 🔧 추가: 위험도별 직원 상세 조회 API
@safety_bp.route('/employees/<risk_level>/detail', methods=['GET'])
def get_employees_by_risk_detail(risk_level):
    """
    위험도별 직원 상세 조회 API (일회성 조회)
    GET /api/web/safety/employees/{risk_level}/detail?token=JWT_TOKEN
    """
    # 토큰 검증
    is_valid, result = verify_token_from_query()
    if not is_valid:
    ...
    # 유효한 위험도 확인
    valid_risk_levels = ['위험', '주의', '정상']
    if risk_level not in valid_risk_levels:
        return jsonify({
            "timestamp": get_korea_now_iso(),
            "status": "error",
            "message": f"유효하지 않은 위험도입니다. 유효한 값: {', '.join(valid_risk_levels)}"
        }), 400
    
    try:
        employees_data = get_employees_by_risk_level_data(risk_level, db.session)
        ...
        print(f"[ERROR] 위험도 {risk_level} 직원 상세 조회 오류: {str(e)}")
        return jsonify({
            "timestamp": get_korea_now_iso(),
            "status": "error",
            "message": f"위험도 {risk_level} 직원 데이터 조회 중 오류가 발생했습니다.",
            "details": str(e)
        }), 500

# 🔧 추가: 위험 상황 통계 API
@safety_bp.route('/statistics/risk-summary', methods=['GET'])
def get_risk_statistics():
    """
    위험 상황 통계 요약 API
    GET /api/web/safety/statistics/risk-summary?token=JWT_TOKEN
    """
    # 토큰 검증
    is_valid, result = verify_token_from_query()
    if not is_valid:
    ...
    try:
        # 위험도별 통계
        risk_stats = {}
        for risk_level in ['위험', '주의', '정상']:
            count = HealthAnomaly.query\
                .filter(HealthAnomaly.risk == risk_level)\
                .filter(HealthAnomaly.status == '처리중')\
                .count()
        ...
        # 증상별 통계
        symptom_stats = {}
        symptoms = db.session.query(HealthAnomaly.symptom)\
            .filter(HealthAnomaly.status == '처리중')\
            .distinct().all()
        
        for symptom_tuple in symptoms:
            symptom = symptom_tuple[0]
            count = HealthAnomaly.query\
                .filter(HealthAnomaly.symptom == symptom)\
                .filter(HealthAnomaly.status == '처리중')\
                .count()
            symptom_stats[symptom] = count
        
        # 총 처리중인 이상 건수
        total_active_anomalies = HealthAnomaly.query\
            .filter(HealthAnomaly.status == '처리중')\
            .count()
        ...
        response_data = {
            ...
            "data": {
                ...
                "total_active_anomalies": total_active_anomalies,
                "summary": {
                    "critical_employees": risk_stats.get('위험', 0),
                    "caution_employees": risk_stats.get('주의', 0),
                    "normal_employees": risk_stats.get('정상', 0)
                }
            },
            "meta": {
                "timezone": "Asia/Seoul",
                "generated_at": get_korea_now_iso()
            }
        }
        ...
        print(f"[ERROR] 위험 상황 통계 조회 오류: {str(e)}")
        return jsonify({
            "timestamp": get_korea_now_iso(),
            "status": "error",
            "message": "위험 상황 통계 조회 중 오류가 발생했습니다.",
            "details": str(e)
        }), 500